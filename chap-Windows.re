...
ショートカットキーを使えると便利だとわかっていながら、使っていない方に向けてまとめてみました。一覧は、公式のHPで見ることができます。

https://support.microsoft.com/ja-jp/help/12445/windows-keyboard-shortcuts

注意点として、OS標準のショートカットキーよりも、現在操作しているアプリケーションのショートカットが優先されます。
...
切り取りとコピーの違いは、選択した項目が消えるか残るかです。この2つのどちらかをすることによって、貼り付けが可能になります。

- Ctrl + V (または Shift + Insert)	選択した項目を貼り付ける。

切り取りもしくはコピーした内容をそのまま貼り付けることができます。この操作は、テキストだけではなくファイルのコピーにも使えます。ソフトウェアで貼り付けを禁止したいとき、Ctrlを禁止するだけではShift+Insertを禁止できないです。Shiftを禁止するわけにいかないので、Insertを禁止することになりますね。

- Ctrl + Z	操作を元に戻す。

操作を一回前に戻すことができます。テキスト入力であれば入力や貼り付け、ファイル操作であれば切り取りや貼り付けをキャンセルできます。しかし、気を付ける必要があります。ファイル操作では、元に戻らない場合があります。USBメモリやネットワークドライブだとファイル削除は完全削除のため、ファイルを削除して戻そうとした場合、戻りません。このような点があるため、ファイル操作に関しては注意して行うべきです。
...
こちらは、Tabキーだけでなく方向キーが使えます。が、Enterキーで確定しないと選択しているアプリが最前面に表示されません。
...
クリップボードは最新だけでなく、実は履歴が記録されています。2つ前、3つ前のコピーした内容を貼り付けられます。
...
ここでいうショートカットメニューとはショートカットキーを押したときに出るメニュー、つまり右クリックメニューです。

- Shift + 任意の方向キー	ウィンドウ内またはデスクトップ上の複数の項目を選択する、またはドキュメント内のテキストを選択する。

選択範囲を押した方向キーに伸ばすことができます。
...
- Esc	現在の作業を停止または終了する。
 ...
 最近のキーボードにはWindowsロゴキー無効化機能が搭載されていることが多く、邪魔者扱いされることが多いキーですが、多くのショートカットキーがあります。
...
Windows10のショートカットキーを紹介しました。よく使うであろうショートカットキーを覚えた後、自分が使うであろうショートカットキーを追加で覚えていくと良いでしょう。ショートカットキーで快適な操作ができることを願っております。